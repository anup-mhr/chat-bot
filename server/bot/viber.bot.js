            ...
            function onUserJoin(value) {
                ...
                newSocket.on("message:received", async (message, metadata) => {
                    ...
                    for (let i = 0; i < responseMessage.length; i++) {
                        try {
                            await handleResponseMessage(this, metadata, responseMessage[i]);
                        } catch (error) {
...
function init(app, bot) {
    ...
    app.post(bot.webhook, async (req, res) => {
        ...
        if (eventName === "conversation_started" && !req.body.subscribed) {
            ...
            const text ="Namaste,\nWelcome to Himalayan Bot!\nI am _Your Virtual Assistant_ .\nPlease use the quick link below or type the queries you may have."
            const custom = [
                {
                  title: "Products",
                  color: "#0171b6",
                  payload: "/product_info",
                  image: "https://himalayanbot.palmchatbot.com/images/menu/productVibera.png",
                },
                {
                  title: "Policy Services",
                  color: "#0171b6",
                  payload: "/policy",
                  image: "https://himalayanbot.palmchatbot.com/images/menu/policyViber.png",
                },
                {
                  title: "Agency",
                  color: "#0171b6",
                  payload: "Agency",
                  image: "https://himalayanbot.palmchatbot.com/images/menu/agencyViber.png",
                },
                {
                  title: "Claim & Loan",
                  color: "#0171b6",
                  payload: "/about_claim",
                  image: "https://himalayanbot.palmchatbot.com/images/menu/claimViber.png",
                },
                {
                  title: "Payment",
                  payload: "/payment",
                  color: "#0171b6",
                  image: "https://himalayanbot.palmchatbot.com/images/menu/paymentViber.png",
                },
                {
                  title: "Contact Us",
                  color: "#0171b6",
                  payload: "/contact",
                  image: "https://himalayanbot.palmchatbot.com/images/menu/contactViber.png",
                },
            ];
...
async function handleResponseMessage(bot, metadata, message) {
    const userId = metadata.receipent.replaceAll('!$', '+')
    if (message.text && message.buttons) {
        await bot.sendReplyButtons(userId, message.text, message.buttons)
    }
    ...
    if (message.text && !message.attachment?.payload && !message.buttons) {
        await bot.sendText(userId, message.text);
    }
    if (!message.custom) {
        return null;
    }
    ...
    if (custom.text || custom.message) {
        return await bot.sendText(userId, custom.text || custom.message);
    }
    ...
    switch (customType) {
        case 'image':
            ...
            await bot.sendMedia(userId, "picture", { media: imageUrl, text: caption });
            break;
        case 'media':
            ...
            await bot.sendMedia(userId, type, data);
            break;
        case 'text':
            await bot.sendText(userId, custom.text || custom.message);
            break
        case 'quickReplyButttons':
            const { replyText, buttons } = custom
            await bot.sendReplyButtons(userId, replyText, buttons)
            break
        case 'location':
            await bot.sendLocation(userId, custom)
            break
        case 'keyboard':
            await bot.sendKeyboard(userId, custom);
            break;
        default:
            await bot.sendRichMedia(userId, custom)
    }
}
