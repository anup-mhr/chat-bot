...
const { patchMessage } = require("../services/message.service");
...
class CustomBootBot extends BootBot {
  ...
  async handleResponseMessageoffline(message, metadata, receptionId) {
    console.log("handleResponseMessage called>>>", message);
    ...
    const displayMessage =
      message.quickReplies || message.buttons
        ? message
        : message.text || message.description || message || "No response";
    message.metadata = { senderId };
    console.log(message, metadata, options, "displayMessage", displayMessage);
    if (message.attachment) {
      const [baseUrl, pathParam] = message.attachment.payload.split("?path=");
      const encodedUrl = `${baseUrl}?path=${encodeURIComponent(pathParam)}`;
      await this.sendAttachment(userId, message.attachment.type, encodedUrl);
    }
    if (message.type === "livechatExpiredRequestOpp") {
      await this.say(receptionId, displayMessage, options);
    }
  ...
  async connectSocket(userId) {
    let prrofileDetails = await this.getUserProfile(userId);
    console.log(prrofileDetails, "proffff>>>>");
    return new Promise(async (resolve, _) => {
      ...
      newSocket.on("connect", () => {
        newSocket.emit(
          ...
          "User",
          this.type,
          { org_id: process.env.NEW_ORGANIZATION_ID, branch_id: process.env.NEW_BRANCH_ID },
          { org_id: process.env.NEW_ORGANIZATION_ID, branch_id: process.env.NEW_BRANCH_ID },
          prrofileDetails,
          onUserJoin.bind(this)
        );
      });
    });
  }
  ...
  async handleResponseMessage(metadata, message) {
    ...
    const displayMessage =
      message.quickReplies || message.buttons
        ? message
        : message.text || message.message || message.description || message || "No response";
    message.metadata = { senderId };
    await this.say(userId, displayMessage, options);
...
function init(app, bot) {
  ...
  const greetingMessage =
    process.env.SET_GREETING_MESSAGE_FACEBOOK ||
    "Warm welcome to Prabhu Life Insurance- Your Virtual Assistant. I am here to help you with your queries related to CG Digital.";
