...
class WhatsappBot extends EventEmitter {
    constructor(config, type = "whatsapp") {
        super();
        if (!config.accessToken || !config.verifyToken || !config.fromId) {
            console.log("missing app credentials", type);
            config = { accessToken: "_____", verifyToken: "______", fromId: "______" };
        }
...
async function handleResponseMessage(bot, metadata, message) {
    ...
    if (message.text && message.buttons) {
        ...
        await bot.sendReplyButtons(userId, replyText, { ...payloadTitleMap });
    }
    ...
    switch (custom.type) {
        case 'image':
            ...
            await bot.sendMedia(userId, "image", { link: imageUrl, caption: caption });
            break;
        case 'media':
        ...
        case 'list':
            ...
            await bot.sendList(userId, buttonName, bodyText, sections, { ...options });
            break;
